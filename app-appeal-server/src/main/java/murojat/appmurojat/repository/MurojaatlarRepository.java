...
public interface MurojaatlarRepository extends JpaRepository<Murojaatlar, UUID> {
    ...
    Page<Murojaatlar> findAllBySourceAndStatusNot(String source, Status status, Pageable pageable);
    ...
    @Query(value = "select count(id) from murojaatlar where status = 'RECEIVE'", nativeQuery = true)
    ...
    @Query(value = "select * from murojaatlar where status = 'RECEIVE' and date(created_at) = (select date(now()) - 3)", nativeQuery = true)
    List<Murojaatlar> getByStatusAndDay3();

    @Query(value = "select * from murojaatlar where status = 'RECEIVE' and date(created_at) = (select date(now()) - 1)", nativeQuery = true)
    List<Murojaatlar> getByStatusAndDay1();

    @Query(value = "select * from murojaatlar where status = 'RECEIVE' and date(created_at) = (select date(now()) - 8)", nativeQuery = true)
